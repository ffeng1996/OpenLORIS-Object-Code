...
parser.add_argument('--fc-layers', type=int, default=3, dest='fc_lay', help="# of fully-connected layers")
parser.add_argument('--fc-units', type=int, default=400, metavar="N", help="# of units in first fc-layers")
...
parser.add_argument('--iters', type=int, default=2000, help="# batches to optimize solver")
parser.add_argument('--lr', type=float, default=0.0001, help="learning rate")
parser.add_argument('--batch', type=int, default=20, help="batch-size")
parser.add_argument('--optimizer', type=str, choices=['adam', 'adam_reset', 'sgd'], default='adam')

parser.add_argument('--feedback', action="store_true", help="equip model with feedback connections")
...
parser.add_argument('--distill', action='store_true', help="use distillation for replay?")
parser.add_argument('--temp', type=float, default=2., dest='temp', help="temperature for distillation")

parser.add_argument('--g-z-dim', type=int, default=100, help='size of latent representation (default: 100)')
parser.add_argument('--g-fc-lay', type=int, help='[fc_layers] in generator (default: same as classifier)')
parser.add_argument('--g-fc-uni', type=int, help='[fc_units] in generator (default: same as classifier)')

parser.add_argument('--g-iters', type=int, help="# batches to train generator (default: as classifier)")
parser.add_argument('--lr-gen', type=float, help="learning rate generator (default: lr)")

parser.add_argument('--ewc', action='store_true', help="use 'EWC' (Kirkpatrick et al, 2017)")
parser.add_argument('--lambda', type=float, default=5000., dest="ewc_lambda",
                    help="--> EWC: regularisation strength")
parser.add_argument('--fisher-n', type=int, help="--> EWC: sample size estimating Fisher Information")
parser.add_argument('--online', action='store_true', help="--> EWC: perform 'online EWC'")
parser.add_argument('--gamma', type=float, default=1., help="--> EWC: forgetting coefficient (for 'online EWC')")
parser.add_argument('--emp-fi', action='store_true', help="--> EWC: estimate FI with provided labels")
parser.add_argument('--si', action='store_true', help="use 'Synaptic Intelligence' (Zenke, Poole et al, 2017)")
parser.add_argument('--c', type=float, default=0.1, dest="si_c", help="--> SI: regularisation strength")
parser.add_argument('--epsilon', type=float, default=0.1, dest="epsilon", help="--> SI: dampening parameter")
parser.add_argument('--xdg', type=float, default=0., dest="gating_prop", help="XdG: prop neurons per layer to gate")

parser.add_argument('--icarl', action='store_true', help="bce-distill, use-exemplars & add-exemplars")
parser.add_argument('--use-exemplars', action='store_true', help="use exemplars for classification")
parser.add_argument('--add-exemplars', action='store_true', help="add exemplars to current task dataset")
parser.add_argument('--budget', type=int, default=2000, dest="budget", help="how many exemplars can be stored?")
parser.add_argument('--herding', action='store_true', help="use herding to select exemplars (instead of random)")
parser.add_argument('--norm-exemplars', action='store_true', help="normalize features/averages of exemplars")

parser.add_argument('--log-per-task', action='store_true', help="set all visdom-logs to [iters]")
parser.add_argument('--loss-log', type=int, default=200, metavar="N", help="# iters after which to plot loss")
parser.add_argument('--prec-log', type=int, default=200, metavar="N", help="# iters after which to plot precision")
parser.add_argument('--prec-n', type=int, default=1024, help="# samples for evaluating solver's precision")
parser.add_argument('--sample-log', type=int, default=500, metavar="N", help="# iters after which to plot samples")
parser.add_argument('--sample-n', type=int, default=64, help="# images to show")
